        ...
        <Container>
          <div className="mx-auto max-w-5xl text-center">
            <Reveal className="flex flex-col items-center">
              ...
              <h1 className="mt-10 text-balance font-serif text-[clamp(2.75rem,7vw,6.5rem)] font-light leading-[1.02] text-ivory">
                The world&rsquo;s longest warranty on the world&rsquo;s first
                engineered hookah.
              </h1>
              <p className="mt-10 max-w-2xl text-pretty text-lg text-ivory-muted">
      ...
      <section className="py-section">
        <Container>
          <div className="grid gap-px overflow-hidden border border-hairline md:grid-cols-3">
            {principles.map((p, i) => (
              <Reveal as="div" key={p.title} delay={i * 0.08} className="bg-ink p-10 md:p-12">
                <p className="font-serif text-4xl font-light text-brass">
                  {String(i + 1).padStart(2, '0')}
                </p>
                <h2 className="mt-8 font-serif text-2xl font-light text-ivory">
                  {p.title}
                </h2>
                <p className="mt-4 text-pretty text-ivory-muted">{p.body}</p>
              </Reveal>
            ))}
          </div>
        </Container>
